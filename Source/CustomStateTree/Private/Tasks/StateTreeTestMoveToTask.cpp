...
#include "Tasks/StateTreeTestMoveToTask.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(StateTreeTestMoveToTask)

#define LOCTEXT_NAMESPACE "CustomStateTree"

EStateTreeRunStatus FStateTreeTestMoveToTask::EnterState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	FInstanceDataType& InstanceData = Context.GetInstanceData(*this);
	if (!InstanceData.AIController)
	{
		UE_VLOG(Context.GetOwner(), LogStateTree, Error, TEXT("FStateTreeTesMoveToTask failed since AIController is missing."));
...
EStateTreeRunStatus FStateTreeTestMoveToTask::Tick(FStateTreeExecutionContext& Context, const float DeltaTime) const
{
	FInstanceDataType& InstanceData = Context.GetInstanceData(*this);
	if (InstanceData.MoveToTask)
	{
		...
		if (InstanceData.bTrackMovingGoal && !InstanceData.TargetActor)
		{
			...
			{
				UE_VLOG(Context.GetOwner(), LogStateTree, Log, TEXT("FStateTreeTesMoveToTask destination has moved enough. Restarting task."));
				return PerformMoveTask(Context, *InstanceData.AIController);
			}
		}
...
void FStateTreeTestMoveToTask::ExitState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	FInstanceDataType& InstanceData = Context.GetInstanceData(*this);
	if (InstanceData.MoveToTask && InstanceData.MoveToTask->GetState() != EGameplayTaskState::Finished)
	{
		UE_VLOG(Context.GetOwner(), LogStateTree, Log, TEXT("FStateTreeTesMoveToTask aborting move to because state finished."));
		InstanceData.MoveToTask->ExternalCancel();
	}
}

UAITask_MoveTo* FStateTreeTestMoveToTask::PrepareMoveToTask(FStateTreeExecutionContext& Context, AAIController& Controller, UAITask_MoveTo* ExistingTask, FAIMoveRequest& MoveRequest) const
{
	FInstanceDataType& InstanceData = Context.GetInstanceData(*this);
...
EStateTreeRunStatus FStateTreeTestMoveToTask::PerformMoveTask(FStateTreeExecutionContext& Context, AAIController& Controller) const
{
	FInstanceDataType& InstanceData = Context.GetInstanceData(*this);
	...
	UE_VLOG(Context.GetOwner(), LogStateTree, Error, TEXT("FStateTreeTesMoveToTask failed because it doesn't have a destination."));
	return EStateTreeRunStatus::Failed;
}

#if WITH_EDITOR
FText FStateTreeTestMoveToTask::GetDescription(const FGuid& ID, FStateTreeDataView InstanceDataView, const IStateTreeBindingLookup& BindingLookup, EStateTreeNodeFormatting Formatting) const
{
